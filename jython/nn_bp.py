...
def main(initlearningRate, fname):
    """Run this experiment"""
    ...
    rule = RPROPUpdateRule(initlearningRate, initlearningRate, initlearningRate)
    oa_names = ["Backprop"]
    ...
    train(BatchBackPropagationTrainer(data_set, classification_network, measure, rule), classification_network,
          'BP', training_ints, measure, TRAINING_ITERATIONS, fname)
...
if __name__ == "__main__":
    ...
    rand.seed(seed)
    learningRate_list = [0.00075, 0.0075, 0.075, 0.75]

    for learningRate in learningRate_list:
        fname = OUTFILE.format('BP', 'Backprop_{}_{}'.format("learningRate", learningRate), str(1))
        with open(fname, 'w') as f:
            f.write(
                '{},{},{},{},{}\n'.format('iterations', 'loss', 'acc_trg', 'f1_trg', 'time'))

        main(learningRate, fname)
